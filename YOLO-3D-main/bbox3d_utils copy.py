...
class BBox3DEstimator:
    ...
    def draw_box_3d(self, image, box_3d, color=(0, 255, 0), thickness=2):
            ...
            Text is displayed at the BOTTOM of the box.
            ...
            # -------------------------------------------------------------------
            # [修改] 文字显示逻辑：移动到框的底部 (Bottom)
            # -------------------------------------------------------------------
            
            # 初始 Y 坐标：设为 y2 (框底部) + 20 像素的间距
            text_y = y2 + 20 
            line_height = 20 # 每行文字的高度间距

            # 1. 绘制 ID (如果有)
            if obj_id is not None:
                cv2.putText(image, f"ID:{obj_id}", (x1, text_y), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
                text_y += line_height # 向下移动一行 (Y增加)
            
            # 2. 绘制 类别名称
            cv2.putText(image, class_name, (x1, text_y), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
            text_y += line_height # 向下移动一行
            
            # 3. 绘制 深度/距离 信息
            if 'depth_value' in box_3d:
                depth_value = box_3d['depth_value']
                # 获取格式化后的距离字符串 (例如: "Mono(3.5m)")
                depth_method = box_3d.get('depth_method', 'unknown')
                
                # 优先显示格式化好的字符串
                if isinstance(depth_method, str) and "Mono" in depth_method:
                    depth_text = depth_method
                else:
