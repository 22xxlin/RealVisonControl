...
class Inference:
    ...
    Updated with Dual-Mode Distance Estimation (Geometric vs Width).
    """
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self.frame_height = 360 # 之前确认是 360p
    ...
    # === [关键修改] 同时计算 Geo 和 Width ===
    def calculate_spatial_info(self, bbox_xyxy, class_id, cam_index=0):
        """
        计算目标的距离和方位角 (返回双重距离)
        :return: dist_geo (m), dist_width (m), azimuth (deg), global_angle (deg)
        """
        ...
        # ---------------- 2. 几何测距法 (Geo-Based) ----------------
        # 仅针对 0-5 类 (小车) 启用，且利用倒立摄像头的 y_max
        dist_geo = None
        ...
        if 0 <= class_id <= 5: 
            ...
            # 融合 IMU Pitch
            total_angle = alpha + self.current_pitch + self.MOUNT_PITCH_OFFSET
        ...
        return dist_geo, dist_width, azimuth_deg, global_angle
    ...
    def sidebar(self) -> None:
        ...
        # 显示实时 Pitch
        self.st.sidebar.metric("Current Pitch (deg)", f"{math.degrees(self.current_pitch):.2f}")
                        ...
                        if frame is not None and frame.size > 0:
                            ...
                            if self.show_distance and results[0].boxes is not None:
                                for box in results[0].boxes:
                                    ...
                                    # 2. 计算双重距离
                                    dist_geo, dist_width, azim, global_ang = self.calculate_spatial_info(
                                        (x1, y1, x2, y2),
                                        class_id,
                                        real_cam_id
                                    )

                                    # 3. 决定显示的 Label 和 3D 深度基准
                                    # 如果有 Geo 距离 (是小车)，用 Geo 做主距离，并显示对比
                                    # 如果是篮球 (Geo为None)，用 Width
                                    primary_dist = dist_geo if dist_geo is not None else dist_width

                                    if dist_geo is not None:
                                        label_str = f"G:{dist_geo:.2f} W:{dist_width:.2f}"
                                    else:
                                        label_str = f"W:{dist_width:.2f}m"

                                    depth_value_norm = self._normalize_distance_to_depth_value(primary_dist)
                                    
                                    box_3d_input = {
                                        'bbox_2d': (x1, y1, x2, y2),
                                        'depth_value': depth_value_norm,
                                        'depth_method': label_str, # <--- 关键：显示对比
    ...
    def _save_yolo_labels(self, result, label_path: Path, img_shape: tuple, cam_id=0) -> None:
        """Save detection results in YOLO format + Dual Distance Info."""
        if not result.boxes:
            return
        ...
        with open(label_path, 'w') as f:
            for box in result.boxes:
                ...
                dist_geo, dist_width, azim, global_ang = self.calculate_spatial_info((x1, y1, x2, y2), class_id, cam_id)
                
                # 处理 Geo 为 None 的情况 (比如篮球)
                d_g = dist_geo if dist_geo is not None else -1.0
                
                # 格式: class x y w h conf dist_geo dist_width azimuth global_angle
                f.write(
                    f"{class_id} {x_center:.6f} {y_center:.6f} {bbox_width:.6f} {bbox_height:.6f} {confidence:.6f} {d_g:.3f} {dist_width:.3f} {azim:.1f} {global_ang:.1f}\n")
